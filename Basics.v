...
(** Consider a different, more efficient representation of natural
    ...
    in the set called [nat],
    and if [n] is in the set then so is [S
...
Fixpoint bin_to_unary (n:bin) : nat :=
  match n with
  | Z => O
  | T x => (bin_to_unary x) + (bin_to_unary x)
  | M x => S (bin_to_unary x)
  end.

(*TESTS*)
Eval compute in (bin_to_unary Z).
Eval compute in (bin_to_unary (T (M Z))).
Eval compute in (bin_to_unary (T (T (M Z)))).
Eval compute in (bin_to_unary (M (T Z))). 
